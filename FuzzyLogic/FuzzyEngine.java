...
public class FuzzyEngine extends FuzzyFrame {
  ...
  */
  ...
  private double synIfValue( ) throws Exception {
    ...
    String op = fExp[idx++];
    String fdN = fExp[idx++];
    if (!fv.contains(fdN)) throw new Exception("Unknown FD: "+fvN+" at line:"+atLine());
    double d = fv.isFuzzy(fdN);
    if ("is".equals(op))   return d;               // is
    if ("some".equals(op)) return d*d;             // some
    if ("very".equals(op)) return Math.sqrt(d);   // very
    if ("not".equals(op))  return (1.0d - d);      // not
    throw new Exception("Unknown Fuzzy-Op: "+op+" at line:"+atLine( ));
   }
