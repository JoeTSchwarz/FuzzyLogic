...
public class FuzzyEngine extends FuzzyFrame {
  ...
  */
  public void execute(String script) throws Exception {
    ...
    if (script.equals(old)) syntax( );
      ...
      String[] tmp = registerFVs().replace("==", "_EQ_").replace("!=", "_NE_")
             .replace(">=", "_GE_").replace("<=", "_LE_").replace("(", " ( ")
             .replace(")", " ) ").replace("+", " + ").replace("-", " - ")
             .replace("*", " * ").replace("/", " / ").replace("=", " = ")
             .replace(",", " , ").replace("\t", " ").replace("||", " || ")
             .replace("&&", " && ").replace(" and ", " && ")
             .replace(" or ", " || ").replace("_EQ_", " == ")
             .replace("_NE_", " != ").replace("_GE_", " >= ")
             .replace("_LE_", " <= ").replaceAll(" ", "\n")
             .split("\n");
      // remove all empty tokens
      ArrayList<String> lst = new ArrayList<>(tmp.length);
      for (String s : tmp) if (s.length() > 0) lst.add(s);
  ...
  */
  ...
  //
  private int skipQuery(int ix) throws Exception {
    ++cx;
    lNo = ix;
    ix = qValue(ix);
  ...
  private int qValue(int ix) throws Exception {
    lNo = ix;
    String op = fExp[ix];
    if (fExp[ix].charAt(0) == '(') return skipQuery(ix+1);
    if (FVs.containsKey(fExp[ix]) || JOs.containsKey(fExp[ix])) return ix+3;
    throw new Exception("Unknown "+fExp[ix]+" at line:"+atLine( ));
  }
  //
  private int cnt, cx;
