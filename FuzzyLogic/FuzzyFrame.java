...
public abstract class FuzzyFrame {
  ...
  */
  ...
  //
  private int skipConditions(int ix) {
    while (ix < fExp.length) {
      if (fExp[ix].charAt(0) == '(') return skipConditions(ix+1);
      if (FVs.containsKey(fExp[ix]) || JOs.containsKey(fExp[ix])) {
        ix += 3; // index to next conditional term. Continue by &&, ||
        if (ix >= fExp.length) return ix; // end of script...
        if (fExp[ix].charAt(0) == '&' || fExp[ix].charAt(0) == '|') ++ix;
        else if (fExp[ix].charAt(0) == ')') return ix+1;       
      } else break;
    }
    return ix;
  }
  /**
  isCondition
  @return boolean following expression is a Condition. Example: a &gt; b &amp;&amp; c == d
  @exception Exception thrown by JAVA
  */
  protected boolean isCondition( ) throws Exception {
    bc  = 0;
    int l = idx;
    boolean boo = true;
    char X = fExp[idx++].charAt(0);
    if (X != '*') { // no endless?
      HashMap<Integer, Boolean> BLst = new HashMap< >( );
      BLst.put(0, false);
      idx = query(BLst, l)-1;
      boo = BLst.get(0);
    }
  ...
  // recursive (a > b || b == c) && (...)
  private int query(HashMap<Integer, Boolean> BLst, int l) throws Exception {
    boolean boo = BLst.get(bc);
    for (int j = 0; ; ) {
      String val = fExp[l++];
      if (val.charAt(0) == '(') {
        BLst.put(++bc, boo);
        ...
        if (fExp[l].charAt(0) != '(') l = conditioning(l);
        else {
          BLst.put(++bc, boo);
          l = query(BLst, l);
          bool = BLst.remove(bc--);
  ...
  //
  private int conditioning(int l) throws Exception {
    int ix = 0;
    bool = false;
    String val = fExp[l++];
    boolean neg = val.charAt(0) == '-';
    ...
    if (ix > 0) { // restore idx and l
      l   = idx;
      idx = ix;
      ix  = 0;
    }
    if (neg) d0 = -d0;           // negative value
    String con = fExp[l++];      // conditional operator
    char C = con.charAt(0);
    if (C != '>' && C != '<' && C != '=' && C != '!')
      throw new Exception("Missing Conditional Operator at line:"+atLine( ));
    val = fExp[l++];             // get varriable
    ...
    if      (con.equals(">"))  bool = d0 >  d1;
    else if (con.equals("<"))  bool = d0 <  d1;
    else if (con.equals("!=")) bool = d0 != d1;
    else if (con.equals("==")) bool = d0 == d1;
    else if (con.equals("<=")) bool = d0 <= d1;
    else if (con.equals(">=")) bool = d0 >= d1;
    else throw new Exception("Missing Conditional Operator at line:"+atLine( ));
    return l;
  }
  //
